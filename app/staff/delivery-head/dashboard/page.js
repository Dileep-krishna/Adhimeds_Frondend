...
export default function DeliveryHeadDashboard() {
  ...
  const assignOrder = (orderId) => {
    ...
    alert(`Order ${orderId} assigned to ${deliveryBoys.find(b => b.id == selectedBoy)?.name}`);
  ...
  return (
    <div>
      ...
      <div className="card p-3 mb-4">
        ...
        <select className="form-select" value={selectedBoy} onChange={e => setSelectedBoy(e.target.value)}>
      ...
      <div className="table-wrapper">
        <table className="data-table">
          <thead><tr><th>Order ID</th><th>Customer</th><th>Address</th><th>Action</th></tr></thead>
          <tbody>
            {readyOrders.map(order => (
              <tr key={order.id}>
                ...
                <td>{order.address}</td>
                <td><button className="btn-sm btn-success" onClick={() => assignOrder(order.id)}>Assign</button></td>
              </tr>
            ))}
          </tbody>
        </table>
